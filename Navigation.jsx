      ...
      <nav style={{ background: 'white', borderBottom: '1px solid #e0e0e0', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
          ...
          {!mobile && (
            <ul style={{ display: 'flex', gap: '0.25rem', listStyle: 'none', margin: 0, padding: 0, flex: 1, marginLeft: '2rem' }}>
              {navLinks.map(({ label, to, href }) => (
                <li key={label}>
                  {to ? <Link to={to} style={linkStyle}>{label}</Link> : <a href={href} style={linkStyle}>{label}</a>}
                </li>
              ))}
            </ul>
          )}
        ...
        {mobile && menuOpen && (
          <div style={{ background: 'white', borderTop: '1px solid #e0e0e0', padding: '1rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
            {navLinks.map(({ label, to, href }) => (
              to ? <Link key={label} to={to} style={mobileLinkStyle} onClick={() => setMenuOpen(false)}>{label}</Link>
...
function AuthNav() {
  ...
  useEffect(() => {
    const handler = (e) => {
      // Only close if click is outside the entire bell + dropdown area
  ...
  const navLinks = [
    ...
    ...(isSuperAdmin ? [{ label: 'Admin', to: '/admin' }, { label: 'Users', to: '/admin/users' }, { label: 'Groups', to: '/admin/groups' }, { label: 'Blog', to: '/admin/blog' }] : []),
  ];
          ...
          {!mobile && (
            <Link to="/profile" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
              ...
              <div style={{ lineHeight: 1.2 }}>
                <div style={{ fontSize: '0.82rem', fontWeight: '600', color: '#1a6b4a' }}>{user?.fullName?.split(' ')[0] || 'Profile'}</div>
                <div style={{ fontSize: '0.68rem', color: '#94a3b8' }}>View profile</div>
              </div>
            </Link>
          )}

          {/* Bell — ref wraps both button AND dropdown */}
          <div ref={dropdownRef} style={{ position: 'relative' }}>
            <button
              onClick={() => setBellOpen(o => !o)}
              style={{ position: 'relative', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '0.4rem 0.6rem', cursor: 'pointer', fontSize: '1.1rem', display: 'flex', alignItems: 'center' }}
            >
              🔔
              {unreadCount > 0 && (
            ...
            {bellOpen && (
              <div
                style={{ position: 'fixed', top: '70px', right: mobile ? '8px' : '16px', left: mobile ? '8px' : 'auto', width: mobile ? 'calc(100vw - 16px)' : '360px', background: 'white', borderRadius: '12px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)', border: '1px solid #e2e8f0', zIndex: 9999, display: 'flex', flexDirection: 'column', maxHeight: '480px' }}
              >
                {/* Header — always visible */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 1rem', borderBottom: '1px solid #f1f5f9', flexShrink: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    <button onClick={() => setBellOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: '1rem', lineHeight: 1, padding: '0 0.25rem' }}>✕</button>
                  </div>
                </div>
                  ...
                  {!notifications?.length ? (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: '0.775rem', color: '#64748b', wordBreak: 'break-word' }}>{n.message}</div>
