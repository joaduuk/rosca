...
function Dashboard() {
  ...
  const [newGroup, setNewGroup] = useState({
    ...
    member_count: 5,
    rosca_type: 'random',
    currency: 'USD' // Add this with a default value
  });
  ...
  const handleCreateGroup = async (e) => {
    ...
    try {
      ...
      const groupData = {
        ...
        currency: newGroup.currency  // Add this line
      };
  ...
  try {
    ...
    // Fix 1: Make sure we have the correct membership_id
    // The member object from cycle-status has 'member_id', not 'membership_id'
    const membershipId = selectedMember.member_id || selectedMember.membership_id || selectedMember.id;
    
    if (!membershipId) {
      alert('Error: Could not determine member ID');
      console.error('Selected member object:', selectedMember);
      return;
    }
    
    // Fix 2: Format date correctly for PostgreSQL/DateTime
    // Send ISO string which is what the backend expects
    const now = new Date();
    const dueDate = now.toISOString(); // Format: 2024-03-10T15:30:00.000Z
    ...
    const response = await API.post(`/groups/${selectedGroup.id}/contributions`, paymentPayload);
    ...
    // Refresh all data
    await fetchMembers(selectedGroup.id);
    await fetchContributions(selectedGroup.id);
    await fetchCycleStatus(selectedGroup.id, currentCycle);
    await fetchPayoutSchedule(selectedGroup.id);
    await fetchContributionSummary(selectedGroup.id);
  ...
  // Use the selected group's currency, fallback to USD if not available
  const currencyCode = selectedGroup?.currency || 'USD';
...
{/* Add this after the stats grid, before the member payment table */}
{currentCycleStatus.paid_count === currentCycleStatus.total_members && 
 currentCycleStatus.payout_status !== 'paid' && (
  <div style={{ 
    marginTop: '1rem', 
    marginBottom: '1rem',
    padding: '1rem',
    ...
    <p style={{ marginBottom: '0.5rem', color: '#276749', fontWeight: 'bold' }}>
      ✅ All members have paid for this cycle!
    </p>
    <button
      onClick={async () => {
        try {
          const response = await API.post(`/groups/${selectedGroup.id}/process-payout/${currentCycleStatus.cycle_number}`);
          alert(`✅ Payout processed! ${response.data.recipient} received ${formatCurrency(response.data.amount)}`);
          
          // Refresh data
          fetchCycleStatus(selectedGroup.id, currentCycleStatus.cycle_number);
          fetchPayoutSchedule(selectedGroup.id);
          fetchContributionSummary(selectedGroup.id);
        } catch (error) {
