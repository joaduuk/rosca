...
export default function Dashboard() {
  ...
  const handleAddMember = async (userId) => {
    if (!selectedGuarantor) {
      alert('Please select a guarantor from the existing members first.');
      return;
    }
    try {
      await API.post(`/groups/${selectedGroup.id}/members/${userId}?is_admin=false&guarantor_user_id=${selectedGuarantor.id}`);
        ...
        <main style={styles.main}>
          {!selectedGroup ? (
            <div style={styles.emptyState}>
              ...
              <h2 style={styles.emptyTitle}>Welcome to ROSCA</h2>
              ...
              {summary && (
                <div style={styles.summaryGrid}>
                  {[
                    ...
                    { label: 'Cycles Completed', value: summary.total_cycles_completed || 0, color: '#7c3aed' },
                  ].map(s => (
                      ...
                      <div style={{ marginBottom: '1.5rem' }}>
                        ...
                        <div style={styles.progressTrack}>
                          <div style={{
                            ...
                            background: cycleStatus.completion_percentage >= 100 ? '#059669' : '#667eea'
                          }} />
                        </div>
                      </div>
          ...
          <div style={styles.modal}>
            ...
            <div style={{ background: '#fef9c3', border: '1px solid #fde047', borderRadius: '8px', padding: '0.875rem', marginBottom: '1rem' }}>
              <p style={{ margin: '0 0 0.5rem', fontWeight: '600', fontSize: '0.85rem', color: '#854d0e' }}>
                🛡️ Step 1 — Select a Guarantor (required)
              </p>
              <p style={{ margin: '0 0 0.75rem', fontSize: '0.78rem', color: '#92400e' }}>
                Choose an existing member who will vouch for the new member. They will be notified by email.
              </p>
              {selectedGuarantor ? (
                ...
                <select
                  ...
                  defaultValue=""
                >
                  <option value="" disabled>— Select guarantor from members —</option>
                  {members.map(m => (
...
const styles = {
  ...
  sidebarCount: { background: '#667eea', color: 'white', borderRadius: '9999px', fontSize: '0.75rem', padding: '0.1rem 0.5rem', fontWeight: '600' },
  newGroupBtn: { width: '100%', padding: '0.6rem', background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '0.875rem' },
  ...
  groupItemActive: { background: 'linear-gradient(135deg, #667eea, #764ba2)', border: '1px solid #667eea', color: 'white' },
  ...
  ctaBtn: { padding: '0.75rem 2rem', background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '1rem' },
  ...
  addMemberBtn: { padding: '0.6rem 1.25rem', background: '#667eea', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', fontSize: '0.875rem', whiteSpace: 'nowrap', flexShrink: 0 },
  ...
  tabActive: { background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', border: '1px solid #667eea' },
  ...
  submitBtn: { padding: '0.6rem 1.25rem', background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600', fontSize: '0.875rem' },
