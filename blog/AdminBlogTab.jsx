...
export default function AdminBlogTab({ token }) {
  ...
  async function fetchPosts() {
    setLoading(true);
    try {
      const r = await fetch(`${API_BASE}/api/blog/posts`, { headers });
  ...
  async function handleSave() {
    ...
    try {
      const url = editingId
        ? `${API_BASE}/api/blog/posts/${editingId}`
      ...
      showFeedback(e.message || "Save failed.", "error");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id) {
    ...
    setDeleting(id);
    try {
      await fetch(`${API_BASE}/api/blog/posts/${id}`, { method: "DELETE", headers });
  ...
  async function handleTogglePublish(post) {
    try {
      await fetch(`${API_BASE}/api/blog/posts/${post.id}`, {
        method: "PUT",
        headers,
  ...
  const s = {
    wrap: { fontFamily: "'DM Sans', sans-serif", color: "#222" },
